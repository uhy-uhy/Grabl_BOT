...
namespace MouseTest1
{
	public class OrderdKeyWatcher
	{
		...
		// イベント
		private event EventHandler<KeyWatcherEventArgs> _callBackMethod;
		...
		public OrderdKeyWatcher(int interval, int threshold, EventHandler<KeyWatcherEventArgs> callBackMethod, params int[] keyCode)
		{
			...
			while (stopFlag)
			{
				if (WatchKey(target.KeyCode))
				{
					...
					if (target.IsTerminal) // 最後のキーが押下されたか?
					{
						...
						{
							// 見つかったらコールバック
							_callBackMethod(this, new KeyWatcherEventArgs(_keyCodeArray));
						}
						intervalSum = 0;
